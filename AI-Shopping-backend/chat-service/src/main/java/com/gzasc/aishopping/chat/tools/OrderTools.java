...
public class OrderTools {
    ...
    public Map<String, Object> getOrderById(@P("订单ID") String orderId, @MemoryId String sessionId) {
    ...
    public List<Map<String, Object>> getAllOrders(@MemoryId String sessionId) {
        Long userId = chatSessionService.getSessionUserId(sessionId);
        return fetchAllOrders(userId);
    }

    @Tool("根据订单状态查询订单列表")
    public List<Map<String, Object>> getOrdersByStatus(@P("订单状态，可选值：PENDING PAID SHIPPED DELIVERED CANCELLED RETURNED") String status, @MemoryId String sessionId) {
        Long userId = chatSessionService.getSessionUserId(sessionId);
