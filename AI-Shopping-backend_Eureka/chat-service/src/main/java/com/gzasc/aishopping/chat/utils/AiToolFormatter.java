...
public class AiToolFormatter {
     ...
     * 格式化订单详细信息（完整信息）
     */
    public static String formatOrderInfo(OrderDTO order, ContactDTO contact) {
     ...
     * 格式化订单概要信息（列表展示用）
     */
    public static String formatOrderSummary(OrderDTO order, ContactDTO contact) {
        String name = contact != null ? contact.getName() : "";
     ...
     * 格式化订单列表
     */
    public static String formatOrders(List<OrderDTO> orders, List<ContactDTO> contacts, String emptyMsg) {
        if (orders == null || orders.isEmpty()) {
            return emptyMsg;
        }
        return orders.stream()
                .map(order -> {
                    ContactDTO contact = findContactById(contacts, order.getContactId());
                    return formatOrderSummary(order, contact);
                })
                .collect(Collectors.joining("\n"));
    }

    private static ContactDTO findContactById(List<ContactDTO> contacts, Integer contactId) {
